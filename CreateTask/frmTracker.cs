...
namespace TimeTracker
{
    ...
    public partial class frmTracker : Form
    {
        ...
        private void SaveGridToFile()
        {
            ...
            DateTime.TryParse(statusStrip1.Items[0].Text, out saveDate);
            ...
            foreach (DataGridViewRow row in dgEntries.Rows)
            {
                ...
                switch (row.Cells["colOperationMode"].Value.ToString())
                {
                    case "Create":
                        ...
                        newItem.StartDate = Convert.ToDateTime(row.Cells["colStartDate"].Value.ToString());
                        newItem.TargetDate = Convert.ToDateTime(row.Cells["colTargetDate"].Value.ToString());
        ...
        private int FindCurrentIterationIndex(List<String> iterationList)
        {
            ...
            foreach (string dateRange in dateRanges)
            {
                ...
                DateTime startDate = DateTime.ParseExact(dates[0], dateFormat, culture);
                DateTime endDate = DateTime.ParseExact(dates[1], dateFormat, culture);
        ...
        private void dgEntries_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            f2.startTime = Convert.ToDateTime(row.Cells[dgEntries.Columns["colStartTime"].Index].Value.ToString());
